...
`Directory.Build.props` redirects `BaseOutputPath` and `BaseIntermediateOutputPath` to `$(USERPROFILE)\Documents\Timberborn\.flood-season-build\`. Must be in `Directory.Build.props`, not the csproj — `BaseIntermediateOutputPath` is read by NuGet before MSBuild reaches the csproj body (MSB3539 warning).

The output path **must be absolute**. An earlier `$(MSBuildThisFileDirectory)..\..\.flood-season-build\` looked correct because it resolved outside the mod folder from the main checkout — but a Claude Code worktree at `Mods/Flood Season/.claude/worktrees/<name>/` resolved the same relative expression to `Mods/Flood Season/.claude/.flood-season-build/`, *inside* the scan radius. The csproj's `CopyToModRoot` `AfterBuild` target also has to be guarded with `Condition="!$(MSBuildProjectDirectory.Contains('.claude'))"` so a worktree build doesn't drop `Code.dll` next to the worktree (which is itself inside the mod folder). After merging a worktree, also `git worktree remove .claude/worktrees/<name>` and delete any stale `Code.dll` files under `.claude/` — the game's recursive scan picks them up regardless of git state.
